...
#[test]
fn known_timestamps() {
    use crate::{Month, Seconds};
    assert_eq!(
        UnixTime::from_datetime(1970, Month::January, 1, 0, 0, 0)
            .unwrap()
    ...
    assert_eq!(
        UnixTime::from_datetime(1970, Month::January, 2, 0, 0, 0)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(1972, Month::January, 1, 0, 0, 0)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(1973, Month::January, 1, 0, 0, 0)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(1976, Month::January, 1, 0, 0, 0)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(2025, Month::July, 16, 16, 23, 24)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(2034, Month::December, 26, 8, 2, 37)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(2760, Month::April, 1, 21, 59, 58)
            .unwrap()
            .elapsed_time_since_epoch(),
    ...
    assert_eq!(
        UnixTime::from_datetime(1643, Month::January, 4, 1, 1, 33)
            .unwrap()
            .elapsed_time_since_epoch(),
...
fn date_roundtrip(year: i32, month: Month, day: u8, hour: u8, minute: u8, second: u8) {
    let time = UnixTime::from_datetime(year, month, day, hour, minute, second).unwrap();
