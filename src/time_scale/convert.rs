...
pub trait FromScale<Scale, Representation, Period> {
...
pub trait IntoScale<Scale, Representation, Period> {
    /// Constructs a time point from an instant expressed in another scale.
    fn into_scale(self) -> TimePoint<Scale, Representation, Period>;
}

impl<S1, R1, P1, S2, R2, P2> IntoScale<S1, R1, P1> for TimePoint<S2, R2, P2>
where
    TimePoint<S1, R1, P1>: FromScale<S2, R2, P2>,
{
    fn into_scale(self) -> TimePoint<S1, R1, P1> {
